# vwc_generator_noth.py
# VWC（植被含水率）单日数据生成工具（不含 Hveg 特征的同步版本）
# 特征顺序: 6VOD + LAI + 10PFTs
# ----------------------------------------------------------------------
...
# from sklearn.ensemble import RandomForestRegressor # 假设模型已预训练
...
class VWCGeneratorNoHveg:
    """
    植被含水率（VWC）日影像生成类（不含 Hveg 特征的同步版本）。
    封装了数据加载、特征准备、模型预测和GeoTIFF输出的整个流程。
    """
    def __init__(self, start_date, end_date, output_path, model_path, 
        ...
        self.logger.info("初始化 VWCGeneratorNoHveg (不含 Hveg)...")
        self._load_model()
        # self._load_hveg_data() 不再需要
    ...
    def _configure_logging(self):
        """配置日志 (保持不变)"""
        # ... (与原文件相同) ...
        self.logger = logging.getLogger('VWCGeneratorNoHveg')
        ...
        if not self.logger.handlers:
            ...
            # 文件输出
            fh = logging.FileHandler(os.path.join(self.output_path, 'vwc_generation_noth.log'))
    ...
    # --- 移除 _load_hveg_data 方法 ---
    
    # --- 其他辅助方法保持不变 (_create_singleband_geotiff, _get_vod_file, _get_lai_file, _get_pft_file, _get_month_centers, _load_lai_matrix, _load_pft_matrix, _create_land_mask) ---

    def _create_singleband_geotiff(self, data, output_path, nodata=-9999.0):
        """创建单波段地理参考的TIFF文件 (保持不变)"""
        try:
    ...
    def _prepare_features(self, vod_data, lai_data, pft_data, valid_mask):
        """
        准备特征矩阵（6VOD + LAI + PFTs）。
        
        注意：移除了 hveg_data 参数和 Hveg 特征。
        """
    ...
    # ------------------------------------------------
    # --- 公共方法：生成VWC影像的主入口（同步版本） ---
    # ------------------------------------------------
    def generate_vwc_maps(self):
        """生成VWC影像（同步版本）"""
        ...
        self.logger.info(f"开始同步处理{len(dates)}天数据")
        
        # 使用 tqdm 包装日期列表以显示进度条
        for date in tqdm(dates, desc="生成VWC影像"):
            try:
                result = self._process_one_date(date)
                if result:
                    completed += 1
                else:
                    failed += 1
            except Exception as e:
                self.logger.error(f"处理日期 {date} 失败: {str(e)}", exc_info=True)
                failed += 1
...
def run_vwc_generation(start_date_str, end_date_str, overwrite=False, **path_kwargs):
    """
    ...
    # 检查所有必要的路径参数 (*** 修改点：移除了 hveg_path ***)
    required_paths = ['output_path', 'model_path', 'vod_base_path', 'lai_base_path', 'pft_base_path']
    ...
    try:
        generator = VWCGeneratorNoHveg( # *** 修改点：使用新类名 ***
        ...
        completed, failed = generator.generate_vwc_maps() # *** 修改点：同步调用 ***
